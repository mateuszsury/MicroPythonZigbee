...
- [x] 2026-02-12 Step 120 (release build + dual-device HIL): rebuilt firmware in WSL (`build_firmware.sh --profile esp32-c6-devkit --skip-mpy-cross`), flashed both ESP32-C6 boards (`COM3`, `COM6`) with full erase + write, validated dual-node high-level scenarios using `tools/hil_stability_suite_runner.py` (`cycles=1`, overlap PASS, color PASS), and prepared release artifacts with checksums in `dist/firmware/uzigbee-esp32c6-20260212/`.
